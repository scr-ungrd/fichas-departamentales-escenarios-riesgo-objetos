---
title: "Objetos de Fichas Departamentales"
---
...
viewof mostrar_burbujas = Inputs.toggle({
  label: "Densidad poblacional municipal",
  value: true
})
...

{
  ...
  // --- Bubble map: densidad de habitantes por municipio ---
  const mun_vis = depto_map === "— Colombia (todos) —"
    ? mun_densidad
    : mun_densidad.filter(d => d.depto === depto_map)

  const local_max = d3.max(mun_vis.filter(d => d.pob > 0), d => d.densidad) || 1
  const r_bub = d3.scaleSqrt(
    [0, local_max],
    [0, depto_map === "— Colombia (todos) —" ? 18 : 30]
  )

  if (mostrar_burbujas) {
    const bubble_tip = container.append("div")
      .style("position",       "absolute")
      .style("pointer-events", "none")
      .style("background",     "rgba(10,40,80,0.88)")
      .style("color",          "white")
    ...
    svg.append("g")
      ...
      .selectAll("circle")
      .data(mun_vis.filter(d => d.pob > 0).sort((a, b) => b.densidad - a.densidad))
      .join("circle")
        .attr("cx", d => path.centroid(d.feature)[0])
        .attr("cy", d => path.centroid(d.feature)[1])
        .attr("r",  d => r_bub(d.densidad))
      .on("mouseover", function(event, d) {
        d3.select(this).attr("fill-opacity", 0.85)
        bubble_tip
          .style("display", "block")
          .html(`<b>${d.nombre}</b> · ${d.depto}<br>
                 <span style="font-size:11px">
                   Densidad: <b>${d.densidad.toFixed(1)} hab/km\u00b2</b><br>
                   Pob: ${d.pob.toLocaleString("es-CO")}
                 </span>`)
      })
      .on("mousemove", function(event) {
        const [mx, my] = d3.pointer(event, container.node())
        bubble_tip.style("left", (mx + 14) + "px").style("top", (my - 48) + "px")
      })
      .on("mouseout", function() {
        d3.select(this).attr("fill-opacity", 0.55)
        bubble_tip.style("display", "none")
      })
  }
